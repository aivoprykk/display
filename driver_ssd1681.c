...
static void epaper_lvgl_flush_cb(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    for (int i = 0; i < len_bits; i++) {
        if(drv->rotated == 1) {
            ...
            // converted_buffer_red[i/8] |= ((((color_map[i].ch.red) > 3) && ((lv_color_brightness(color_map[((i*len_x)%len_bits) + i/len_y])) > 250)) << (7-(i % 8)));
        }
        else {
...
esp_lcd_panel_handle_t display_ssd1681_new() {
    ...
    if(err) {
        ESP_LOGE(TAG, "display_ssd1681_new err: %s", esp_err_to_name(err));
    }
    ...
    epaper_panel_register_event_callbacks_ssd1681(panel_handle, &cbs, &disp_drv);
...
#endif // CONFIG_DISPLAY_DRIVER_SSD1681