...
static lv_obj_t *load(lv_obj_t *parent) {
    ...
    for (int i = 0, ypos = 0; i < numrows; ++i, ypos += h) {
        ...
        if (i == 6) {
            lv_obj_set_height(rows[i], lv_pct(18));
            lv_obj_set_y(rows[i], lv_pct(84));
            lv_obj_set_style_pad_left(rows[i], 5, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_pad_right(rows[i], 0, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_pad_top(rows[i], 0, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_pad_bottom(rows[i], 0, LV_PART_MAIN | LV_STATE_DEFAULT);
            lv_obj_set_style_text_font(rows[i], ui_sleep_screen.font.title, LV_PART_MAIN | LV_STATE_DEFAULT);

            lv_obj_t *title_lbl = lv_label_create(rows[i]);
            lv_obj_set_width(title_lbl, LV_SIZE_CONTENT);   /// 1
            lv_obj_set_height(title_lbl, LV_SIZE_CONTENT);  /// 1
            lv_obj_set_align(title_lbl, LV_ALIGN_LEFT_MID);
