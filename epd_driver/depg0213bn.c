...
static const uint8_t lut_full[] = {
    0x50, 0xAA, 0x55, 0xAA, 0x11, 0x00, 0x00, 0x00, //LUT0: BB:     VS 0 ~7
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //LUT1: BW:     VS 0 ~7
    0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0x1F, 0x00, //LUT2: WB:     VS 0 ~7
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, //LUT3: WW:     VS 0 ~7
};
...
static void Init_Part(void* _me, uint8_t em) {
    ...
    epd->op->writeCommand(_me, SSD1680_CMD_SET_END_OPTION);
    ...
    epd->op->writeByte(_me, 0x00);
    epd->op->writeByte(_me, 0x00);
    epd->op->writeByte(_me, 0x40);
    epd->op->writeByte(_me, 0x00);
    epd->op->writeByte(_me, 0x00);
    epd->op->writeByte(_me, 0x00);
    epd->op->writeByte(_me, 0x00);

    epd->op->writeCommand(_me, SSD1680_CMD_SET_BORDER_WAVEFORM);
    epd->op->writeByte(_me, SSD1680_PARAM_BORDER_WAVEFORM);
    epd->op->writeCommand(_me, SSD1680_CMD_SET_DISP_UPDATE_CTRL);
    epd->op->writeByte(_me, 0xC0); // enable clk signal enable analog
    epd->op->writeCommand(_me, SSD1680_CMD_ACTIVE_DISP_UPDATE_SEQ);
