...
#include "../../../ssd168x_waveshare_2in13_lut.h"
static const char *TAG = "display_drv.ssd168x";
#endif
#ifdef CONFIG_DISPLAY_DRIVER_SSD1681
#include "../../../ssd168x_waveshare_1in54_lut.h"
static const char *TAG = "display_drv.ssd168x";
#endif
...
void drawImg(esp_lcd_panel_handle_t panel_handle, const uint8_t *img, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t rotation, bool swap, bool invert_color, bool partial)
{
    ...
    // ESP_ERROR_CHECK(epaper_panel_set_bitmap_color_ssd168x(panel_handle, SSD1680_EPAPER_BITMAP_RED));
    // ESP_ERROR_CHECK(esp_lcd_panel_draw_bitmap(panel_handle, x, y, w, h, img));
    ESP_ERROR_CHECK(epaper_panel_set_bitmap_color_ssd168x(panel_handle, SSD1680_EPAPER_BITMAP_BLACK));
    ESP_ERROR_CHECK(esp_lcd_panel_draw_bitmap(panel_handle, x, y, w, h, img));
...
void app_main(void)
{
    ...
    esp_lcd_ssd168x_config_t epaper_config = {
        ...
        // since those operations are not supported by ssd168x and are implemented by software
    ...
    esp_lcd_panel_dev_config_t panel_config = {
        ...
        .vendor_config = &epaper_config
    };
    esp_lcd_panel_handle_t panel_handle = NULL;
    // NOTE: Please call gpio_install_isr_service() manually before esp_lcd_new_panel_ssd168x()
    // because gpio_isr_handler_add() is called in esp_lcd_new_panel_ssd168x()
    ...
    // cbs does not have to be static for ssd168x driver, for the callback ptr is copied, not pointed
    ...
    // epaper_panel_clear_screen_ssd168x(panel_handle);
    ESP_LOGI(TAG, "Drawing speed_raw_200x200 0, false, false, true");
    drawImg(panel_handle, speed_raw_200x200, 0, 0, 200, 200, 0, false, false, true);
    vTaskDelay(pdMS_TO_TICKS(2000));
    
    ESP_LOGI(TAG, "Drawing speed_raw_250x122 0, true, false, true");
    drawImg(panel_handle, speed_raw_200x200, 0, 0, 200, 200, 0, true, false, true);
    vTaskDelay(pdMS_TO_TICKS(2000));

    // epaper_panel_clear_screen_ssd168x(panel_handle);
    ESP_LOGI(TAG, "Drawing speed_raw_200x200 1, false, false, true");
    drawImg(panel_handle, speed_raw_200x200, 0, 0, 200, 200, 1, false, false, true);
    vTaskDelay(pdMS_TO_TICKS(2000));

    ESP_LOGI(TAG, "Drawing speed_raw_200x200 1, true, false, true");
    drawImg(panel_handle, speed_raw_200x200, 0, 0, 200, 200, 1, true, false, true);
    vTaskDelay(pdMS_TO_TICKS(2000));

    // epaper_panel_clear_screen_ssd168x(panel_handle);
    ESP_LOGI(TAG, "Drawing speed_raw_200x200 2, false, false, true");
    drawImg(panel_handle, speed_raw_200x200, 0, 0, 200, 200, 2, false, false, true);
    vTaskDelay(pdMS_TO_TICKS(2000));

    ESP_LOGI(TAG, "Drawing speed_raw_200x200 2, true, false, true");
    drawImg(panel_handle, speed_raw_200x200, 0, 0, 200, 200, 2, true, false, true);
    vTaskDelay(pdMS_TO_TICKS(2000));
    
    // epaper_panel_clear_screen_ssd168x(panel_handle);
    ESP_LOGI(TAG, "Drawing speed_raw_200x200 3, false, false, true");
    drawImg(panel_handle, speed_raw_200x200, 0, 0, 122, 250, 3, false, false, true);
    vTaskDelay(pdMS_TO_TICKS(2000));

    ESP_LOGI(TAG, "Drawing speed_raw_200x200 3, true, false, true");
    drawImg(panel_handle, speed_raw_200x200, 0, 0, 200, 200, 3, true, false, true);
    vTaskDelay(pdMS_TO_TICKS(2000));
    ...
    // vTaskDelay(100 / portTICK_PERIOD_MS);
    // epaper_panel_clear_screen_ssd168x(panel_handle, 0x0);
